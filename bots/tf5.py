    ...
    async with session.get(url, params=query_params) as response:
        if response.status == 200:
            ...
            if not data.empty:
                ...
                if data['signal'][4] > 1.0:
                    send_signal_message(pair, data['signal'][4], "LONG", green_heart)
                elif data['signal'][4] < -1.0:
                    send_signal_message(pair, data['signal'][4], "SHORT", red_heart)
        else:
